...
if (!$apt) { http_response_code(404); site_head('Not found'); echo '<main class="container mx-auto px-4 py-12">Not found</main>'; site_footer(); exit; }
...
<main class="min-h-[60vh] bg-[#FAF7EE]">
  <div class="container mx-auto px-4 py-6 md:py-8">
    <a href="<?php echo url('apartments.php'); ?>" class="inline-flex items-center gap-2 text-sm text-gray-700 hover:text-gray-900">
    ...
    <p class="mt-1 text-gray-600">
      ...
      <?php if (!empty($apt['sofaBeds'])): ?> · <?php echo (int)$apt['sofaBeds']; ?> sofa bed<?php echo $apt['sofaBeds']>1?'s':''; ?><?php endif; ?>
      ...
      <section>
        <div class="relative rounded-xl bg-white border border-mediterranean-sand p-3">
          <?php
            ...
            $images = $apt['images'] ?? [];
            $featured = null;
            foreach ($images as $im) { if (!empty($im['featured']) || !empty($im['isFeatured'])) { $featured = $im; break; } }
            if (!$featured && !empty($images)) $featured = $images[0];
          ?>
          <?php if ($featured): ?>
            <div class="relative">
          ...
          <?php if (!empty($images)): ?>
            <div id="thumbs" class="mt-3 flex gap-3 overflow-x-auto pb-1" role="listbox" aria-label="Gallery thumbnails">
              <?php foreach ($images as $i => $im): ?>
                <button class="relative shrink-0 w-[220px] h-[130px] rounded-lg overflow-hidden border-2 <?php echo $i===0?'border-blue-300':'border-transparent'; ?>"
        ...
        <div class="mt-4 grid grid-cols-3 gap-3">
          <button class="tab-btn rounded-lg bg-[#EDE7DA] hover:bg-[#E2D7C3] py-2 px-3 font-medium" data-tab="desc">Description</button>
          <button class="tab-btn rounded-lg bg-[#EDE7DA] hover:bg-[#E2D7C3] py-2 px-3" data-tab="amen">Amenities</button>
          <button class="tab-btn rounded-lg bg-[#EDE7DA] hover:bg-[#E2D7C3] py-2 px-3" data-tab="rules">House Rules</button>
        </div>
          ...
          <div id="panel-amen" class="hidden rounded-xl bg-white border border-mediterranean-sand p-4">
            <div class="flex flex-wrap gap-2">
              <?php foreach (($apt['amenities'] ?? []) as $am): ?>
                <span class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-sm"><?php echo htmlspecialchars($am); ?></span>
              <?php endforeach; ?>
            </div>
          </div>
      ...
      <aside class="sticky top-4 h-fit">
        <div class="rounded-xl bg-white border border-mediterranean-sand p-5">
          ...
          <div class="mt-4">
            ...
            <input id="dateRange" class="w-full rounded-md border border-mediterranean-sand px-3 py-2 text-sm" placeholder="Select dates" />
          </div>
          ...
          <div class="mt-3">
            ...
            <select id="guests" class="w-full rounded-md border border-mediterranean-sand bg-white px-3 py-2 text-sm">
              <?php $max = max(1,(int)($apt['guests'] ?? 4)); for ($g=1;$g<=$max;$g++): ?>
          ...
          <form method="post" action="/booking/create.php" class="mt-4">
            ...
            <button class="w-full rounded-lg bg-mediterranean-blue text-white py-3 font-semibold disabled:opacity-60" id="bookBtn" disabled>
              Book Now
            </button>
          </form>
